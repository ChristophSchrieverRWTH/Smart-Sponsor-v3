...
class App extends Component {
  ...
  updateCoins = async () => {   // call this method after moving coins
    try{
      ...
      const maxID = parseInt(await this.state.bankC.methods.coinID().call());
      let wallet = [];
      for(var i = 0; i < maxID; i++){
        if(parseInt(response[i].coinID) < maxID){
          let curr = response[i];
          let cutCoin = {coinID: parseInt(curr.coinID), senderConditions: curr.senderConditions, receiverConditions: curr.receiverConditions, permit: ""};
          wallet.push(cutCoin);
        }
      }
      if(wallet.length === 0){
  ...
  render() {
    ...
    if (this.state.website==='bank') {
      activeWebsite = <Bank wallet={this.state.coins} onPermit={this.permit}/>;
    ...
    return (
    <div className="m-3">
      <NavComp active={this.state.website} onClick={this.changeActive}/>
      {activeWebsite}
    </div>
    );
  };
